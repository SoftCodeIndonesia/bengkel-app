...
class VehicleController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('merk', function ($row) {
                    return $row->vehicle->merk;
                })
                ->addColumn('tipe', function ($row) {
                    return $row->vehicle->tipe;
                })
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href="' . route('vehicles.show', $row->vehicle_id) . '" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">';
                    $btn .= '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>';
                    $btn .= '</a>';
                    $btn .= '<a href="' . route('vehicles.edit', $row->vehicle_id) . '" class="p-2 text-green-600 hover:bg-green-50 rounded-lg">';
                    ...
                    $btn .= '<button type="button" data-id="' . $row->id . '" data-name="' . $row->customer->name . '" data-vehicle="' . $row->vehicle->merk . ' ' . $row->vehicle->tipe . '" class="delete-vehicle p-2 text-red-600 hover:bg-red-50 rounded-lg">';
    ...
    {
        $vehicle->load('customers');
        $customer = $vehicle->customers->first();

        return view('vehicles.show', compact('vehicle', 'customer'));
    }
