...
@section('content')
    <div class="bg-gray-800 shadow overflow-hidden">
        <div class="p-4 border-b border-gray-600">
            <h2 class="text-xl font-semibold text-white">Edit Stok Opname</h2>
        </div>

        <form action="{{ route('stock-opname.update', $opname->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-4 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" id="opname_date" name="opname_date"
                            value="{{ old('opname_date', $opname->opname_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2"
                        class="w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('notes', $opname->notes) }}</textarea>
                </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var opname = @json($opname);
            console.log(opname);

            opname.items.forEach(element => {
                const item = {
                    product_id: element.product.id,
                    product_name: element.product.name,
                    system_stock: element.system_stock,
                    physical_stock: element.physical_stock,
