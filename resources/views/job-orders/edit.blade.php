            ...
            <form action="{{ route('job-orders.update', $jobOrder->id) }}" method="POST" id="jobOrderForm">
                ...
                <div class="w-full" id="field-customer_vehicle_id">
                    ...
                    <div class="relative w-full">
                        <input type="hidden" name="customer_vehicle_id" value="{{ old('customer_vehicle_id', $jobOrder->customerVehicle->id) }}" />
                        <input type="text" id="customer_vehicle_name" value="{{ old('customer_name', $jobOrder->customerVehicle->customer->name) }}" name="customer_vehicle_name"
                        ...
                        <button type="button" id="modal-select-customer"
                            class="absolute top-0 end-0 p-2.5 h-full text-sm font-medium text-white bg-gray-500 rounded-e-lg border border-gray-500 hover:bg-gray-500 focus:ring-4 focus:outline-none focus:ring-blue-300 ">
                            ...
                            <div>
                                ...
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <p class="text-white font-medium" id="customer-name">{{ $jobOrder->customerVehicle->customer->name }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-300">Telepon:</p>
                                        <p class="text-white font-medium" id="customer-phone">{{ $jobOrder->customerVehicle->customer->phone ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-300">Email:</p>
                                        <p class="text-white font-medium" id="customer-email">{{ $jobOrder->customerVehicle->customer->email ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-300">Alamat:</p>
                                        <p class="text-white font-medium" id="customer-address">{{ $jobOrder->customerVehicle->customer->address ?? '-' }}</p>
                                    </div>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <p class="text-white font-medium" id="vehicle-merk">{{ $jobOrder->customerVehicle->vehicle->merk ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-300">Tipe:</p>
                                        <p class="text-white font-medium" id="vehicle-type">{{ $jobOrder->customerVehicle->vehicle->tipe ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-300">Nomor Polisi:</p>
                                        <p class="text-white font-medium" id="vehicle-plate">{{ $jobOrder->customerVehicle->vehicle->no_pol }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex gap-6">
                        <div class="flex-1">
                            <div class="mb-4" id="field-customer_name">
                                <label for="name" class="block text-sm font-medium text-gray-300">Nama Lengkap <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-4">
                                <label for="address" class="block text-sm font-medium text-gray-300">Alamat</label>
                        ...
                        <div class="flex-1">
                            <div class="mb-4" id="field-merk">
                                <label for="merk" class="block text-sm font-medium text-gray-300">Merk <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-4" id="field-tipe">
                                <label for="tipe" class="block text-sm font-medium text-gray-300">Tipe <span class="text-red-500">*</span></label>
                            ...
                            <div id="field-no-pol">
                                <label for="no_pol" class="block text-sm font-medium text-gray-300">Nomor Polisi <span class="text-red-500">*</span></label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 my-6">
                    <div>
                        <label for="km" class="block text-sm font-medium text-gray-300 mb-2">Kilometer <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label for="service_at" class="block text-sm font-medium text-gray-300 mb-2">Tanggal Servis <span class="text-red-500">*</span></label>
                        <input type="datetime-local" value="{{ old('service_at', optional($jobOrder->service_at)->format('Y-m-d\TH:i')) }}"
                            ...
                            @foreach (old('breakdowns', $jobOrder->breakdowns) as $breakdown)
                                <div class="breakdown-row flex gap-4 mb-3">
                                    <div class="col-span-11 flex-1">
                                        ...
                                        <input type="hidden" name="breakdowns[{{ $breakIndex }}][id]" value="{{ $breakdown['id'] ?? '' }}">
                                    </div>
                            ...
                            <tbody id="service-items-container">
                                @foreach ($jobOrder->orderItems as $item)
                                    @if ($item->product->tipe == 'jasa')
                                        @php
                                            $basePrice = 100000;
                                            $diskonPerUnit = $basePrice * ($item->diskon_value / 100);
                                            $unitPriceAfterDiscount = $basePrice - $diskonPerUnit;
                                            $subtotal = $unitPriceAfterDiscount;
                                            $feePerUnit = $subtotal * ($item->fee_value / 100);
                                            $markupPrice = $subtotal + $feePerUnit;
                                            $total = $markupPrice * $item->quantity;
                                            $feeAmount = $feePerUnit * $item->quantity;
                                            $diskonAmount = $diskonPerUnit * $item->quantity;
                                        @endphp
                                        <tr class="border-b border-gray-600 item-row" data-tipe="{{ $item->product->tipe }}" data-index="{{ $itemIndex }}">
                                            <td class="p-2" width="300px">
                                                <input type="hidden" name="items[{{ $itemIndex }}][id]" value="{{ $item->id }}">
                                                <input type="hidden" name="items[{{ $itemIndex }}][type]" value="jasa">
                                                <input type="hidden" name="items[{{ $itemIndex }}][product_id]" value="{{ $item->product_id }}">
                                                <span class="product-name text-gray-300">{{ $item->product->name }}</span>
                                            </td>
                                            <td class="p-2 text-center">
                                                <span class="kategori text-gray-300">jasa</span>
                                            </td>
                                            <td class="p-2 text-center">
                                                <span class="qty-display text-gray-300">{{ $item->quantity }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][quantity]" class="quantity-hidden" value="{{ $item->quantity }}">
                                                <input type="hidden" class="base-price-hidden" value="{{ $basePrice }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="unit-price text-gray-300">Rp {{ number_format($basePrice, 0, ',', '.') }}</span>
                                            </td>
                                            <td class="p-2 text-center">
                                                <span class="diskon-display text-gray-300">{{ $item->diskon_value }}%</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][diskon_value]" class="diskon-hidden" value="{{ $item->diskon_value }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="diskon-amount-display text-gray-300">Rp {{ number_format($diskonPerUnit, 0, ',', '.') }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][diskon_amount]" class="diskon-amount-hidden" value="{{ $diskonAmount }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="subtotal-display text-gray-300">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][subtotal]" class="subtotal-hidden" value="{{ $subtotal }}">
                                            </td>
                                            <td class="p-2 text-center">
                                                <span class="fee-display text-gray-300">{{ $item->fee_value }}%</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][fee_value]" class="fee-hidden" value="{{ $item->fee_value }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="fee-amount-display text-gray-300">Rp {{ number_format($feePerUnit, 0, ',', '.') }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][fee_amount]" class="fee-amount-hidden" value="{{ $feeAmount }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="markup-price-display text-gray-300">Rp {{ number_format($markupPrice, 0, ',', '.') }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][markup_price]" class="markup-price-hidden" value="{{ $markupPrice }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="total-display text-gray-300">Rp {{ number_format($total, 0, ',', '.') }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][total]" class="total-hidden" value="{{ $total }}">
                                            </td>
                                            <td class="p-2 text-center" width="100px">
                                                <button type="button" class="edit-item text-blue-500 hover:text-blue-400 mr-2" data-index="{{ $itemIndex }}">
                                    ...
                                    @if ($item->product->tipe != 'jasa')
                                        ...
                                        <tr class="border-b border-gray-600 item-row" data-tipe="{{ $item->product->tipe }}" data-index="{{ $itemIndex }}">
                                            <td class="p-2 text-left" width="300px">
                                                <input type="hidden" name="items[{{ $itemIndex }}][id]" value="{{ $item->id }}">
                                                <input type="hidden" name="items[{{ $itemIndex }}][type]" value="barang">
                                                <input type="hidden" name="items[{{ $itemIndex }}][product_id]" value="{{ $item->product_id }}">
                                                <span class="product-name text-gray-300">{{ $item->product->name }}</span>
                                            </td>
                                            <td class="p-2 text-center">
                                                <span class="grade text-gray-300">{{ $item->product->grade ?? '-' }}</span>
                                            </td>
                                            <td class="p-2 text-center">
                                            ...
                                            <td class="p-2 text-center">
                                                <span class="qty-display text-gray-300">{{ $item->quantity }}</span>
                                                <input type="hidden" name="items[{{ $itemIndex }}][quantity]" class="quantity-hidden" value="{{ $item->quantity }}">
                                                <input type="hidden" name="items[{{ $itemIndex }}][buying_price]" value="{{ $item->buying_price ?? 0 }}">
                                                <input type="hidden" class="base-price-hidden" value="{{ $basePrice }}">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span class="unit-price text-gray-300">Rp {{ number_format($basePrice, 0, ',', '.') }}</span>
                ...
                <div class="mb-6">
                    <label for="notes" class="block text-sm font-medium text-gray-300">Catatan</label>
                    <textarea name="notes" id="notes" rows="3"
                        class="mt-1 block w-full bg-gray-700 text-white border border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('notes', $jobOrder->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div class="p-3 border-t border-gray-700 flex justify-end space-x-2">
                <button type="button" id="cancel-edit"
                    class="px-3 py-1.5 text-sm bg-gray-600 text-white rounded-lg hover:bg-gray-500">Batal</button>
                <button type="button" id="save-edit"
                    class="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">Simpan</button>
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ✅ AMBIL DATA JOB ORDER DARI SERVER
        var job_order = @json($jobOrder);
        // ✅ SET itemCounter = jumlah item yang sudah ada + 1
        itemCounter = job_order.order_items.length + 1;
        ...
        // ✅ FUNCTION addItemRow - SAMA PERSIS DENGAN CREATE
        function addItemRow(kategori, productName, grade, productId, unit_price, quantity, buying_price) {
            ...
            if (tipe === 'barang') {
                ...
                row.innerHTML = `
                    ...
                    <td class="p-2 text-right">
                        <span class="unit-price text-gray-300">Rp ${formatNumber(base_price)}</span>
        ...
        // ✅ FUNCTION addItemRowPackage - SAMA PERSIS DENGAN CREATE
        function addItemRowPackage(type, containerId, data) {
            ...
            if (type === 'barang') {
                row.innerHTML = `
                    ...
                    <td class="p-2 text-right">
                        <span class="subtotal-display text-gray-300">Rp ${formatNumber(data.unit_price)}</span>
                        <input type="hidden" name="items[${itemCounter}][subtotal]" class="subtotal-hidden" value="${data.unit_price}">
                    </td>
                    <td class="p-2 text-center">
                        <span class="fee-display text-gray-300">0%</span>
                        <input type="hidden" name="items[${itemCounter}][fee_value]" class="fee-hidden" value="0">
                    ...
                    <td class="p-2 text-right">
                        <span class="total-display text-gray-300">Rp ${formatNumber(data.total)}</span>
        ...
        // ✅ FUNCTION initItemRowEvents - SAMA PERSIS DENGAN CREATE
        function initItemRowEvents(row, type) {
            ...
            // ✅ REMOVE BUTTON - DENGAN KONFIRMASI UNTUK EXISTING ITEM
            const removeBtn = row.querySelector('.remove-item');
            if (removeBtn) {
                removeBtn.addEventListener('click', function() {
                    const itemId = row.querySelector('input[name*="[id]"]')?.value;
                    if (itemId && !itemId.toString().startsWith('delete_')) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                row.querySelector('input[name*="[id]"]').value = 'delete_' + itemId;
        ...
        document.querySelectorAll('.item-row').forEach(row => {
            const type = row.dataset.tipe || 'barang';
            initItemRowEvents(row, type);
        });

        // ✅ FUNCTION calculateTotal - SAMA PERSIS DENGAN CREATE
        ...
        document.getElementById('jobOrderForm').addEventListener('submit', function(e) {
            ...
            // console.log('ppn amount', originalNumber($('input[name="ppn_amount"]').val()));
            // e.preventDefault();
            const customer_vehicle_id = document.querySelector('input[name="customer_vehicle_id"]').value;
            ...
            $('input[name="total_sparepart"]').val(originalNumber($('input[name="total_sparepart"]').val()));
            ...
            $('input[name="total_diskon_item"]').val(originalNumber($('input[name="total_diskon_item"]').val()));
            // e.preventDefault();
        });
        ...
        var table = $('#product-table-list').DataTable({
            ...
            columns: [
                { data: 'checkbox', name: 'checkbox', orderable: false, searchable: false, className: 'px-4 py-3' },
                { data: 'name', name: 'name', className: 'px-4 py-3' },
                { data: 'grade', name: 'grade', className: 'px-4 py-3' },
                { data: 'formatted_price', name: 'unit_price', className: 'px-4 py-3' },
                { data: 'stok', name: 'stok', className: 'px-4 py-3' }
            ],
            ...
            initComplete: function() {
                ...
                $('.dataTables_filter input').addClass('bg-gray-700 border border-gray-600 text-white rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500');
                $('.dataTables_length select').addClass('bg-gray-700 border border-gray-600 text-green-600 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500');
                $('.dataTables_processing').css({ 'background': 'transparent', 'color': 'white' });
            },
            drawCallback: function() {
                $('.dataTables_info').addClass('text-gray-400');
                $('.pagination-container .paginate_button').addClass('px-3 py-1 mx-1 text-gray-300 bg-gray-700 border border-gray-600 rounded-md hover:bg-gray-600 hover:text-white transition duration-150');
                $('.pagination-container .paginate_button.current').addClass('bg-blue-600 text-white border-blue-600');
        ...
        var tableCustomer = $('#customer-table-list').DataTable({
            ...
            columns: [
                ...
                { data: 'customer_name', name: 'customer_name', className: 'px-4 py-1' },
                { data: 'phone', name: 'phone', className: 'px-4 py-1' },
                { data: 'address', name: 'address', className: 'px-4 py-1' },
                { data: 'kendaraan', name: 'kendaraan', className: 'px-4 py-1' },
                { data: 'no_pol', name: 'no_pol', className: 'px-4 py-1' },
                { data: 'action', name: 'action', className: 'px-4 py-1' }
            ],
            dom: '<"flex flex-col md:flex-row justify-between items-center mb-4"<"mb-2 md:mb-0"l><"flex items-center"f>>rt<"flex flex-col md:flex-row justify-between items-center mt-4"<"mb-2 md:mb-0"i><"pagination-container"p>>',
            initComplete: function() {
                $('.dataTables_length label').addClass('text-gray-400');
                ...
                fetch(`${base_url}/api/customer_vehicles/${id}/details`)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            document.getElementById('vehicle-merk').textContent = data.vehicle.merk || '-';
                            document.getElementById('vehicle-type').textContent = data.vehicle.tipe || '-';
                            document.getElementById('vehicle-plate').textContent = data.vehicle.no_pol || '-';
                            ...
                            if (document.getElementById('tipe')) {
                                document.getElementById('tipe').value = data.vehicle.tipe || '';
                            }
        ...
        document.getElementById('confirm-selection').addEventListener('click', function() {
            ...
            checkboxes.forEach(checkbox => {
                ...
                if (!selectedProducts.includes(productId)) {
                    ...
                    addItemRow(kategori, productName, grade, productId, originalNumber(productPrice), quantity, originalNumber(buying_price));
                }
            });
