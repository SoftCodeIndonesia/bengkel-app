    ...
    <div class="bg-gray-800 rounded-lg shadow overflow-hidden border border-gray-600">
        ...
        <div class="bg-gray-800 rounded-lg shadow p-6">
            <form action="{{ route('job-orders.update', $jobOrder->id) }}" method="POST" id="jobOrderForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 my-6">
                    ...
                    <div>
                        ...
                        <label for="service_at" class="block text-sm font-medium text-gray-300 mb-2">Tanggal Servis <span
                                class="text-red-500">*</span></label>
                        <input type="datetime-local" name="service_at" id="service_at" required
                ...
                {{-- ===================== SPAREPART ===================== --}}
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-300 mb-3">Sparepart (Barang)</h3>
                    <table class="min-w-full divide-y divide-gray-600 bg-gray-700 text-white text-sm"
                        ...
                        <thead class="text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th class="p-2 text-end">Harga Satuan</th>
                                ...
                                <tr class="border-b border-gray-600 item-row " data-tipe="{{ $item->product->tipe }}">
                                    <td class="p-2" width="200px">
                                        ...
                                        <input type="text" name="items[{{ $itemIndex }}][name]" min="1"
                                            ...
                                            class="bg-gray-700 border border-gray-600 text-white rounded-md py-1 px-2 w-full">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div class="col-span-1">
                            <div class="flex justify-between mb-2">
                                ...
                                <input type="text" name="total_sparepart" id="total-sparepart"
                                    value="Rp {{ number_format($total_sparepart, 2, ',', '.') }}"
                            ...
                            <div class="flex justify-between mb-2">
                                ...
                                <input type="text" name="total_jasa" id="total-jasa"
                                    value="Rp {{ number_format($total_service, 2, ',', '.') }}"
                                    class="bg-gray-700 border-none text-end text-white rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                    readonly>
                            </div>
                            <div class="flex justify-between mb-2">
                                <span class="text-gray-300">Total Diskon:</span>
                                <input type="text" name="total_diskon_item" id="total-diskon-item"
                                    value="Rp {{ number_format($total_diskon, 2, ',', '.') }}"
                                    class="bg-gray-700 border-none text-end text-white rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                    readonly>
                            </div>
                        </div>
                    ...
                    <div class="border-t border-gray-600 pt-2 mt-2">
                        <div class="flex justify-between text-lg font-medium">
                            ...
                            <input type="text" name="total" id="total"
                                value="Rp {{ number_format($jobOrder->total, 2, ',', '.') }}"
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // document.getElementById('subtotal').value = 'Rp ' + formatNumber(document.getElementById('subtotal')
            //     .value);
            // document.getElementById('diskon-text').value = 'Rp ' + formatNumber(document.getElementById(
            //     'diskon-text').value);
            // document.getElementById('total').value = 'Rp ' + formatNumber(document.getElementById('total').value);
        ...
        // `;
            ...
            function addItemRow(type) {
                ...
                row.innerHTML = `
                    ...
                    <td class="p-2" width="100px">
                        <input type="number" name="items[${itemCounter}][quantity]" min="1" value="1" ${type == 'jasa' ? 'step="0.1"' : ''}
                            class="quantity bg-gray-700 border border-gray-600 text-white rounded-md py-1 px-2 w-full">
                    </td>
                    <td class="p-2 text-right">
            ...
            function itemRowInit(row) {
                const typeInput = row.querySelector('.kategori');
                const qtyInput = row.querySelector('.quantity');
                const priceText = row.querySelector('.unit-price');
                const subtotalText = row.querySelector('.subtotal');
                const totalAfterDiskon = row.querySelector('.total-after-diskon');
                const diskonValue = row.querySelector('.diskon-value');


                const unit_price = parseFloat(priceText.textContent.replace('Rp ', '').replace(/\./g, ''));
                const subtotal = (unit_price * qtyInput.value);

                const diskon = parseFloat(diskonValue.value);
                const afterDiskon = subtotal * (1 - (diskon / 100));

                console.log('diskon', diskon);
                console.log('subtotal', diskon);

                totalAfterDiskon.textContent = `Rp ${formatNumber(afterDiskon)}`;
            ...
            $('.diskon-value').on('input', function(e) {
                console.log('ok');
                e.preventDefault();
            ...
            $('.quantity').on('input', function(e) {
                ...
                var row = this.closest('tr'); // ambil baris saat ini
                itemRowInit(row);

            });


            document.querySelectorAll('.item-row', function(row) {
                console.log(row);
                // initItemRowEvents(row)
            })
            ...
            //     const totalAfterDiskonCell = row.querySelector('.total-after-diskon');
            ...
            //     // Update display
            //     unitPriceCell.textContent = 'Rp ' + formatNumber(unitPrice);
            //     subtotalCell.textContent = 'Rp ' + formatNumber(subtotal);
            //     totalAfterDiskonCell.textContent = 'Rp ' + formatNumber(totalAfterDiskon);
            ...
            function calculateTotal() {
                ...
                document.querySelectorAll('.item-row').forEach(row => {
                    if (row.style.display !== 'none') {
                        ...
                        const diskon = (subtotalValue - totalAfterDiskon);

                        console.log('subtotal text', subtotalValue);
                        console.log('subtotal text', totalAfterDiskon);
            ...
            $('#jobOrderForm').submit(function(e) {
                ...
                const total_sparepart = parseFloat($('input[name="total_sparepart"]')
                    .val().replace('Rp ', '').replace(/\./g, ''));
                const total_jasa = parseFloat($('input[name="total_jasa"]')
                    .val().replace('Rp ', '').replace(/\./g, ''));
                const total_diskon_item = parseFloat($('input[name="total_diskon_item"]')
                    .val().replace('Rp ', '').replace(/\./g, ''));
                const total = parseFloat($('input[name="total"]')
                    .val().replace('Rp ', '').replace(/\./g, ''));

                // console.log(total_sparepart);

                $('input[name="total_sparepart"]').val(total_sparepart);
                $('input[name="total_jasa"]').val(total_jasa);
                $('input[name="total_diskon_item"]').val(total_diskon_item);
            ...
            // Initialize event listeners for existing breakdown remove buttons
            document.querySelectorAll('.remove-breakdown').forEach(button => {
                button.addEventListener('click', function() {
                    const row = this.closest('.breakdown-row');
                    const breakdownId = row.querySelector('input[name*="[id]"]').value;

                    console.log(breakdownId);

                    if (breakdownId) {
                        // Existing breakdown - mark for deletion
                        row.querySelector('input[name*="[id]"]').value = 'delete_' + breakdownId;
                        row.classList.add('to-be-deleted');
                        row.style.display = 'none';
                    } else {
                        // New breakdown - just remove
                        row.remove();
                    }
                });
            });
